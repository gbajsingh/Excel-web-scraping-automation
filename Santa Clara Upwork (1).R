...
# setting work directory to read and write desired excel file
# setwd("C:/Users/bajsingh/Desktop/Upwork job 1/Santa Clara")
...
search = function(Texts){
  ...
  for(j in Texts){
    ...
    nextpage = submit_form(html_doc, set_values(html_form(html_doc)[[1]], search_token=j ,location='Santa Clara County, CA'))                                   # submit form to get to next/resulting page
    ...
    if(nextpage %>% html_nodes(xpath = '//title') %>% html_text() == "Lookup | PropertyShark"){
      ...
      # if next page's title is "UI | PropertyShark" meaning listed
